...
def calculate_final_estimate(stats: Dict, complexity: float, current_progress: Dict) -> float:
    ...
    # Используем максимальное базовое время из активных шагов
    base_time = max(get_standard_time(step) for step in active_steps)
    ...
    if steps_history:
        ...
        if parallel_time > 0:
            parallel_delay = parallel_time / max(
                get_standard_time(step) for step in active_steps
            )
    ...
    progress_factor = max(steps_time.get(step, 0) for step in active_steps) / base_time
